...
static lv_obj_t *create(hmi_widget_t *w, lv_obj_t *parent)
{
    ...
    grid_state_t *st = lv_malloc_zeroed(sizeof *st);
    st->obj = obj;
    w->state = st;
    return obj;
}
...
{
    (void)w;
    grid_state_t *st = w->state;
    if (!st) return;
    lv_obj_t *obj = st->obj;

    if (strcmp(prop, "spacing") == 0) {
        int s = (int)hmi_value_as_num(value, 8);
        lv_obj_set_style_pad_column(obj, s, 0);
        lv_obj_set_style_pad_row(obj, s, 0);
    }
}
...
const hmi_widget_ops_t hmi_widget_grid = {"Grid", create, set_prop, destroy};