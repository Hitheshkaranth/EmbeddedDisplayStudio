...
`shell/` sits **beside** `gui/`, not inside it: the loader resolves its shell as
`Path(__file__).parent.parent / "shell" / "Shell.qml"`. Installing the QML into
`gui/` instead produces an image that boots, fails to load the shell, exits, and
is restarted for ever by `Restart=always` — with no UI and no fallback screen.

Both interpreters (`hmi-install`, `hmi-gui-launch`, `hmi-hwd-launch`) resolve
