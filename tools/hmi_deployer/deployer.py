...
def validate_bundle(bundle_dir: str) -> Tuple[bool, List[str]]:
    ...
    # Qt binding. Only meaningful for native Python apps: a QML bundle is
    # loaded by the platform's own loader, which is PySide6 either way.
    binding = manifest.get("qt_binding", "pyside6")
    if runtime == "python" and "qt_binding" in manifest:
        # Absence is not an error: manifests written before this field existed
        # still validate, and the caller fills it in. A *wrong* value is an
        # error, because it starts the wrong interpreter on the panel.
        if binding not in ("pyside2", "pyside6"):
            errors.append("manifest.json: 'qt_binding' must be 'pyside2' or 'pyside6'.")
        elif isinstance(entry, str) and entry.endswith(".py") and not errors:
            # Declaring the wrong binding starts the wrong interpreter and the
            # app dies on its first import, so it is worth catching here.
            detected = detect_qt_binding(bundle_dir, entry)
            if detected != binding:
                errors.append(
                    f"manifest.json: 'qt_binding' says '{binding}' but the sources import "
                    f"{'PySide2' if detected == 'pyside2' else 'PySide6'}."
                )
...
def detect_qt_binding(bundle_dir: str, entry: str) -> str:
    """
    Works out which Python Qt binding an application imports.
    ...
    pattern = re.compile(r"^\s*(?:import|from)\s+(PySide2|PySide6)\b", re.M)

    def bindings_in(path: str) -> set:
        try:
            with open(path, "r", encoding="utf-8", errors="replace") as fh:
                # Imports live at the top; the resource modules generated by
                # rcc are tens of megabytes of base64 and reading them whole
                # would dominate the import of a large bundle.
                return set(pattern.findall(fh.read(65536)))
        except OSError:
            return set()

    entry_path = os.path.join(bundle_dir, entry)
    found = bindings_in(entry_path) if entry.endswith(".py") else set()
    if found:
    ...
    for root, dirs, files in os.walk(bundle_dir):
        ...
        for fname in files:
            if not fname.endswith(".py"):
                continue
            for binding in bindings_in(os.path.join(root, fname)):
                counts[binding] += 1

    if counts["PySide2"] > counts["PySide6"]:
        return "pyside2"
    return "pyside6"
